...
class DatabaseEloquentSubqueriesCrossDatabaseTest extends TestCase
{
    ...
    public function testWithCountAcrossDatabaseConnection()
    {
        ...
        $this->assertEquals('select `users`.*, (select count(*) from `mysql2`.`'.$this->tablesPrefix.'orders` as `orders` where `users`.`id` = `orders`.`user_id` and `name` like ?) as `orders_count` from `users`', $query->toSql());
        ...
        $this->assertEquals('select "users".*, (select count(*) from "pgsql2"."'.$this->tablesPrefix.'orders" as "orders" where "users"."id" = "orders"."user_id" and "name" like ?) as "orders_count" from "users"', $query->toSql());
        ...
        $this->assertEquals('select [users].*, (select count(*) from [sqlsrv2].['.$this->tablesPrefix.'orders] as [orders] where [users].[id] = [orders].[user_id] and [name] like ?) as [orders_count] from [users]', $query->toSql());
